/* ============================================================
   site.js · 全典站点：hash 路由、章节目录、⌘K、栅格覆盖、对比度计算器、图表引擎
   ============================================================ */
(function () {
  'use strict';
  ...
  let current = null;
  function activate({ path, anchor }, { push = false } = {}) {
    ...
    const swap = () => {
      ...
      buildToc(target); window.sinoArt?.renderAll(target); drawCharts(target);
      if (anchor) { const el = document.getElementById(anchor); if (el) { el.scrollIntoView({ behavior: 'auto', block: 'start' }); return; } }
      if (current !== target) scrollTo({ top: 0, behavior: 'auto' });
    };
    if (current && current !== target && document.startViewTransition && !matchMedia('(prefers-reduced-motion: reduce)').matches) document.startViewTransition(swap); else swap();
    current = target; closeSheet();
  }

  /* ── 移动端导航抽屉 ── */
  const sheet = $('#nav-sheet'); const closeSheet = () => { sheet?.classList.remove('is-open'); $('.site-nav__menu')?.setAttribute('aria-expanded', 'false'); };
  $('.site-nav__menu')?.addEventListener('click', () => { const on = !sheet.classList.contains('is-open'); sheet.classList.toggle('is-open', on); $('.site-nav__menu').setAttribute('aria-expanded', String(on)); });
  sheet?.addEventListener('click', ev => { if (ev.target === sheet || ev.target.closest('a')) closeSheet(); });

  /* ── 本章目录 + 滚动高亮 ── */
  function buildToc(route) {
    const subs = $$('.cx-sub[id]', route); const list = $('[data-toc]', route); const inline = $('[data-toc-inline]', route);
    const items = subs.map(s => ({ id: s.id, n: $('.cx-sub__n', s)?.textContent || '', t: $('.cx-sub__t', s)?.textContent || '' }));
    const link = it => { const a = document.createElement('a'); a.href = `#/${route.dataset.route}/${it.id}`; a.innerHTML = `<span class="toc__n"></span><span class="toc__t"></span>`; a.children[0].textContent = it.n; a.children[1].textContent = it.t; a.dataset.for = it.id; return a; };
    if (list) { list.replaceChildren(...items.map(link)); }
    if (inline) { inline.replaceChildren(...items.slice(0, 12).map(it => { const a = link(it); a.className = 'cover__idx'; return a; })); }
    io?.disconnect();
    if (list && 'IntersectionObserver' in window) {
      io = new IntersectionObserver(es => { const vis = es.filter(e => e.isIntersecting).sort((a, b) => a.boundingClientRect.top - b.boundingClientRect.top); if (vis[0]) $$('a', list).forEach(a => a.classList.toggle('is-active', a.dataset.for === vis[0].target.id)); }, { rootMargin: '-12% 0px -75% 0px' });
  ...
  const items = [...routes.map(r => ({ href: `#/${r.dataset.route === 'home' ? '' : r.dataset.route}`, num: (D.routes || []).find(x => x.path === r.dataset.route)?.num || '', title: r.dataset.title, kind: '章节' })), ...$$('.cx-sub[id]').map(s => ({ href: `#/${s.closest('.route').dataset.route}/${s.id}`, num: $('.cx-sub__n', s)?.textContent || '', title: $('.cx-sub__t', s)?.textContent || '', kind: '小节' }))];
  let cursor = 0, shown = [];
  const render = q => { const s = q.trim().toLowerCase(); shown = items.filter(i => !s || (i.title + i.num + i.kind).toLowerCase().includes(s)).slice(0, 14); cursor = 0; list.innerHTML = shown.length ? '' : '<div class="cx-cmdk__empty">未找到匹配结果。试试调整关键词？</div>'; shown.forEach((i, idx) => { const b = document.createElement('a'); b.href = i.href; b.className = 'cx-cmdk__item' + (idx === 0 ? ' is-active' : ''); b.innerHTML = `<span class="toc__n"></span><span></span><small></small>`; b.children[0].textContent = i.num; b.children[1].textContent = i.title; b.children[2].textContent = i.kind; b.addEventListener('click', closeCmdk); list.appendChild(b); }); };
  ...
  document.addEventListener('keydown', ev => {
    ...
    if (ev.key === 'Enter' && shown[cursor]) { location.hash = shown[cursor].href; closeCmdk(); }
  });
  ...
  /* ── 首页：实体面板交互 ── */
  $$('[data-entity-panel]').forEach(p => { p.addEventListener('click', () => { document.dispatchEvent(new CustomEvent('sino:set-entity', { detail: p.dataset.entityPanel })); $$('[data-entity-panel]').forEach(x => x.classList.toggle('is-on', x === p)); }); });
  document.addEventListener('sino:entity', ev => $$('[data-entity-panel]').forEach(x => x.classList.toggle('is-on', x.dataset.entityPanel === ev.detail)));

  /* ── 首页数字滚动 ── */
  if ('IntersectionObserver' in window && !matchMedia('(prefers-reduced-motion: reduce)').matches) {
    const nio = new IntersectionObserver(es => es.forEach(e => { if (!e.isIntersecting) return; const el = e.target; const end = Number(el.dataset.count); const t0 = performance.now(); const dur = 1100; const step = now => { const k = Math.min(1, (now - t0) / dur); const v = Math.round(end * (1 - Math.pow(1 - k, 3))); el.textContent = v.toLocaleString('zh-CN'); if (k < 1) requestAnimationFrame(step); }; requestAnimationFrame(step); nio.unobserve(el); }), { threshold: .5 });
    $$('[data-count]').forEach(el => nio.observe(el));
  }
  drawCharts(document);
  addEventListener('hashchange', () => activate(parse()));
  activate(parse());
})();
