/* ============================================================
   compendium.js · 全典外壳：目录高亮、⌘K、对比度计算器、图表、样张辅助
   ============================================================ */
(function () {
  'use strict';
  ...
  /* ── 目录：滚动高亮 + 面包屑 ── */
  const links = $$('.cx-rail__link[href^="#"]');
  const sections = links.map(l => document.getElementById(l.getAttribute('href').slice(1))).filter(Boolean);
  const crumb = $('[data-crumb]');
  const setActive = id => { links.forEach(l => l.classList.toggle('is-active', l.getAttribute('href') === '#' + id)); const s = document.getElementById(id); if (crumb && s) { const t = s.dataset.title || s.querySelector('.cx-head__title')?.textContent || ''; crumb.innerHTML = `<span>全典</span><svg class="a-icon a-icon--18 m-breadcrumb__sep"><use href="#i-chevron-right"/></svg><b></b>`; crumb.querySelector('b').textContent = t; } };
  if ('IntersectionObserver' in window && sections.length) {
    const io = new IntersectionObserver(es => { const vis = es.filter(e => e.isIntersecting).sort((a, b) => a.boundingClientRect.top - b.boundingClientRect.top); if (vis[0]) setActive(vis[0].target.id); }, { rootMargin: '-15% 0px -70% 0px', threshold: 0 });
    sections.forEach(s => io.observe(s));
  }
  /* 移动端目录 */
  const rail = $('.cx-rail'), scrim = $('.cx-scrim');
  const toggleRail = on => { rail?.classList.toggle('is-open', on); scrim?.classList.toggle('is-open', on); $('.cx-menu-btn')?.setAttribute('aria-expanded', String(on)); };
  $('.cx-menu-btn')?.addEventListener('click', () => toggleRail(!rail.classList.contains('is-open')));
  scrim?.addEventListener('click', () => toggleRail(false));
  rail?.addEventListener('click', ev => { if (ev.target.closest('a[href^="#"]') && innerWidth < 1240) toggleRail(false); });

  /* ── ⌘K 命令面板 ── */
  const cmdk = $('#cmdk'); const input = $('#cmdk-input'); const list = $('#cmdk-list');
  const items = [...links.map(l => ({ id: l.getAttribute('href').slice(1), num: l.querySelector('.cx-rail__num')?.textContent || '', title: l.textContent.replace(l.querySelector('.cx-rail__num')?.textContent || '', '').trim(), kind: '章节' })), ...$$('.cx-sub[id]').map(s => ({ id: s.id, num: s.querySelector('.cx-sub__n')?.textContent || '', title: s.querySelector('.cx-sub__t')?.textContent || '', kind: '小节' }))];
  let cursor = 0, shown = [];
  const render = q => {
    const s = q.trim().toLowerCase(); shown = items.filter(i => !s || (i.title + i.num + i.kind).toLowerCase().includes(s)).slice(0, 14); cursor = 0;
    list.innerHTML = shown.length ? '' : '<div class="cx-cmdk__empty">未找到匹配结果。试试调整关键词？</div>';
    shown.forEach((i, idx) => { const b = document.createElement('button'); b.type = 'button'; b.className = 'cx-cmdk__item' + (idx === 0 ? ' is-active' : ''); b.innerHTML = `<span class="cx-rail__num"></span><span></span><small></small>`; b.children[0].textContent = i.num; b.children[1].textContent = i.title; b.children[2].textContent = i.kind; b.addEventListener('click', () => go(i.id)); list.appendChild(b); });
  };
  const go = id => { closeCmdk(); document.getElementById(id)?.scrollIntoView({ behavior: 'smooth', block: 'start' }); };
  const openCmdk = () => { if (!cmdk) return; cmdk.classList.add('is-open'); cmdk.removeAttribute('aria-hidden'); input.value = ''; render(''); setTimeout(() => input.focus(), 20); };
  const closeCmdk = () => { cmdk?.classList.remove('is-open'); cmdk?.setAttribute('aria-hidden', 'true'); };
  window.sinoCmdk = openCmdk;
  document.addEventListener('keydown', ev => {
    ...
    if (ev.key === 'Enter' && shown[cursor]) go(shown[cursor].id);
  });
  ...
  window.sinoContrast = contrast;
  const tool = $('#contrast-tool');
  if (tool) {
    const fg = $('[name="fg"]', tool), bg = $('[name="bg"]', tool), fgH = $('[name="fgHex"]', tool), bgH = $('[name="bgHex"]', tool);
    const out = $('.contrast-tool__ratio', tool), prev = $('.contrast-tool__preview', tool);
    ...
    [fg, bg].forEach(i => i.addEventListener('input', upd));
    [[fgH, fg], [bgH, bg]].forEach(([h, c]) => h.addEventListener('change', () => { if (/^#?[0-9a-f]{6}$/i.test(h.value)) { c.value = '#' + h.value.replace('#', ''); upd(); } }));
    upd();
  }
  ...
  function lineChart(host, cfg) {
    ...
    series.forEach((s, si) => {
      const col = s.color || `var(--c-series${si + 1})`;
    ...
    const tip = el('foreignObject'); host.replaceChildren(svg);
    const tipEl = document.createElement('div'); tipEl.className = 'o-chart__tip'; host.appendChild(tipEl);
    const hit = el('rect', { x: m.l, y: m.t, width: iw, height: ih, fill: 'transparent' }); svg.appendChild(hit);
    const show = ev => {
      const r = svg.getBoundingClientRect(); const px = ((ev.clientX - r.left) / r.width) * W; const i = Math.max(0, Math.min(labels.length - 1, Math.round(((px - m.l) / iw) * (labels.length - 1))));
      cross.setAttribute('x1', x(i)); cross.setAttribute('x2', x(i)); cross.style.opacity = 1;
      dots.forEach((d, si) => { d.setAttribute('cx', x(i)); d.setAttribute('cy', y(series[si].data[i])); d.setAttribute('opacity', 1); });
      tipEl.innerHTML = `<b></b>` + series.map((s, si) => `<div><i style="--sw:${s.color || `var(--c-series${si + 1})`}"></i><span class="v"></span> <span class="n"></span></div>`).join('');
      tipEl.querySelector('b').textContent = labels[i]; tipEl.querySelectorAll('div').forEach((d, si) => { d.querySelector('.v').textContent = fmt(series[si].data[i]) + unit; d.querySelector('.n').textContent = series[si].name; });
      const hr = host.getBoundingClientRect(); tipEl.style.left = `${((x(i) / W) * hr.width).toFixed(0)}px`; tipEl.style.top = `${((Math.min(...series.map(s => y(s.data[i]))) / H) * hr.height).toFixed(0)}px`; tipEl.classList.add('is-on');
    };
  ...
  function barChart(host, cfg) {
    ...
    const svg = el('svg', { viewBox: `0 0 ${W} ${H}`, role: 'img', 'aria-label': cfg.title || '柱状图' });
    const grid = el('g', { class: 'grid' }), axis = el('g', { class: 'axis' }); svg.append(grid, axis);
    ...
    if (horizontal) {
      ...
      labels.forEach((l, i) => {
        axis.appendChild(el('text', { x: m.l - 8, y: m.t + band * i + band / 2 + 4, 'text-anchor': 'end' }, l));
        series.forEach((s, si) => { const v = s.data[i]; const w = (v / vMax) * iw; const yy = m.t + band * i + (band - bh * series.length) / 2 + si * (bh + 2); const r = el('rect', { x: m.l, y: yy, width: Math.max(w, 0), height: bh, fill: (cfg.colors && series.length === 1) ? cfg.colors[i] : (s.color || `var(--c-series${si + 1})`), class: 'bar', tabindex: 0 }); r.dataset.v = fmt(v) + unit; r.dataset.n = `${l} · ${s.name}`; svg.appendChild(r); if (series.length === 1) svg.appendChild(el('text', { x: m.l + w + 6, y: yy + bh / 2 + 4, class: 'label' }, fmt(v) + unit)); });
      });
    } else {
      ...
      labels.forEach((l, i) => {
        axis.appendChild(el('text', { x: m.l + band * i + band / 2, y: H - 8, 'text-anchor': 'middle' }, l));
        series.forEach((s, si) => { const v = s.data[i]; const h = (v / vMax) * ih; const xx = m.l + band * i + (band - (bw * series.length + 2 * (series.length - 1))) / 2 + si * (bw + 2); const r = el('rect', { x: xx, y: m.t + ih - h, width: bw, height: Math.max(h, 0), fill: (cfg.colors && series.length === 1) ? cfg.colors[i] : (s.color || `var(--c-series${si + 1})`), class: 'bar', tabindex: 0 }); r.dataset.v = fmt(v) + unit; r.dataset.n = `${l} · ${s.name}`; svg.appendChild(r); if (cfg.labelMax && v === allMax) svg.appendChild(el('text', { x: xx + bw / 2, y: m.t + ih - h - 6, 'text-anchor': 'middle', class: 'label label--strong' }, fmt(v) + unit)); });
      });
    }
  ...
  function buildTable(id, labels, series, unit) {
    ...
    const t = document.createElement('table'); t.className = 'a-table a-table--data a-table--dense';
  ...
  /* 渲染所有声明式图表 */
  const drawChart = host => { try { const cfg = JSON.parse(host.dataset.chart); (cfg.type === 'bar' ? barChart : lineChart)(host, cfg); } catch (e) { console.error('chart', e); } };
  $$('[data-chart]').forEach(drawChart);
  if ('ResizeObserver' in window) { let t; const ro = new ResizeObserver(es => { clearTimeout(t); t = setTimeout(() => es.forEach(e => { const w = Math.round(e.contentRect.width); if (w && e.target.dataset.w !== String(w)) { e.target.dataset.w = String(w); drawChart(e.target); } }), 120); }); $$('[data-chart]').forEach(h => ro.observe(h)); }
  $$('[data-spark]').forEach(host => sparkline(host, host.dataset.spark.split(',').map(Number)));

  /* ── 雷达图（EDU 学生档案） ── */
  $$('[data-radar]').forEach(host => {
    const d = JSON.parse(host.dataset.radar); const n = d.axes.length, R = 78, cx = 110, cy = 100;
    const svg = el('svg', { viewBox: '0 0 220 200', class: 'p-radar', role: 'img', 'aria-label': '能力雷达' });
    [0.25, 0.5, 0.75, 1].forEach(k => svg.appendChild(el('polygon', { class: 'ring', points: d.axes.map((_, i) => { const a = -Math.PI / 2 + (i * 2 * Math.PI) / n; return `${(cx + Math.cos(a) * R * k).toFixed(1)},${(cy + Math.sin(a) * R * k).toFixed(1)}`; }).join(' ') })));
    d.axes.forEach((ax, i) => { const a = -Math.PI / 2 + (i * 2 * Math.PI) / n; svg.appendChild(el('line', { class: 'axis', x1: cx, y1: cy, x2: cx + Math.cos(a) * R, y2: cy + Math.sin(a) * R })); svg.appendChild(el('text', { x: cx + Math.cos(a) * (R + 16), y: cy + Math.sin(a) * (R + 16) + 4, 'text-anchor': 'middle' }, ax)); });
    svg.appendChild(el('polygon', { class: 'area', points: d.values.map((v, i) => { const a = -Math.PI / 2 + (i * 2 * Math.PI) / n; return `${(cx + Math.cos(a) * R * v / 100).toFixed(1)},${(cy + Math.sin(a) * R * v / 100).toFixed(1)}`; }).join(' ') }));
    host.replaceChildren(svg);
  });
